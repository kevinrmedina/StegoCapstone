...
class Hex_GUI(object):
	...
	def setupUi(self):
		...
		self.scrollArea.setGeometry(QtCore.QRect(280, 20, 411, 411))# self.scrollArea.setWidgetResizable(True)
		...
		self.label.setGeometry(QtCore.QRect(26, 16, 361, 371))# self.label.setFrameShape(QtWidgets.QFrame.Box)
	...
	def retranslateUi(self, Form):
		...
		self.Form.setWindowTitle(_translate("Form", "Hexdump"))
	...
	def openFile(self):
		...
		if fileExtension == 'png' or fileExtension == 'jpg' or fileExtension == 'jpeg' or fileExtension == 'bmp' or fileExtension == 'gif':
			self.label.setPixmap(image)# self.label.resize(image.width(), image.height())
		else :
	...
	def saveFile(self):
		...
		if self.fileName2:
			file = self.fileName
		subprocess.call(["./UIFiles/Hexdump3.py", file, self.fileName2])

	#
	#app = QtWidgets.QApplication(sys.argv)# Form = QtWidgets.QWidget()# ui = Hex_GUI()# ui.setupUi(Form)# Form.show()# sys.exit(app.exec_())